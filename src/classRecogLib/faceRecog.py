...
def loadDetectionModel():
    ...
    mp_face_detection = mp.solutions.face_detection.FaceDetection(model_selection=0, min_detection_confidence=0.5).process
...
def findFaces(image, faceDetector):
    ...
    if results.detections:
        for detection in results.detections:
            ...
            newX, newY = getNewLocations([(xMin+w/2),(yMin+h/2)],centerImage,-angle)
            crop = imageC[int(newY-h/2):int(newY+h/2) , int(newX-w/2):int(newX+w/2)]
...
def encodeFace(face_image,encoderModel ,num_jitters=1):
    ...
    encoding = face_encoder.compute_face_descriptor(face_image, raw_landmarks, num_jitters)
    return encoding